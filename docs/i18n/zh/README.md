...
上图是一整轮、未剪辑、2.5 倍速（[同一轮的 MP4，原速](../../media/walk-to-the-living-room.mp4)）。指令是 "Walk to the living room."。System 2 是 gpt-5.5：它先记下任务，透过头部相机看，记一条「前方是带大窗和白色座椅的明亮房间，应该是客厅」，然后调了三次 `move_forward`。System 1 是身体节点上的步态策略，把每次调用变成 50 Hz 的关节目标，并回报实测距离——2.20 m、2.21 m、1.20 m。到门口时大脑记下高窗、白沙发、绿植和木地板，回答 "I'm in the living room."。左下角的追拍相机是操作者的，大脑从头到尾只看到左上角那些帧。

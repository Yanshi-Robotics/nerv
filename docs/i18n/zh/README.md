...
<div align="center"><img src="../../images/walk-to-the-living-room.gif" alt="gpt-5.5 收到「走到客厅」：记下所见、用步态策略走 2 m、2 m、1.2 m，认出沙发、玻璃墙和绿植，报告到达" width="960"></div>

上图除了加速（2.5 倍）没有剪辑：指令是「走到客厅」。System 2 是 gpt-5.5：它先记下任务，透过头部相机看，记一条「前方是带大窗户的开放空间，右侧房间像卧室」，然后调了三次 `move_forward`。System 1 是身体节点上的步态策略，把每次调用变成 50 Hz 的关节目标，并回报实测距离。大脑认出沙发、玻璃墙和绿植，回答「已到达客厅」。左下角的追拍相机是操作者的，大脑从头到尾只看到左上角那些帧。
